...
const fetchChildren = async () => {
  ...
  } catch (e) { console.error("Failed to fetch children") }
}
...
const handleAdjustment = async (type) => {
  ...
  await axios.post(`${API_BASE}/adjust/${activeAdjustId.value}`, null, {
    params: { amount: finalAmount, description: adjustReason.value || "Admin Adjustment", category: adjustCategory.value, password: token }
  })
  closeModal(); fetchChildren()
}

const closeModal = () => { activeAdjustId.value = null; adjustAmount.value = 0; adjustReason.value = ''; }
...
<template>
  <div class="admin-view">
    ...
    <div class="tile">
      ...
      <table>
        <thead>
          <tr><th>Name</th><th>Balance</th><th class="text-right">Actions</th></tr>
        </thead>
        <tbody>
    ...
    <div class="tile wish-admin-tile">
      ...
      <div class="wish-list-table">
        ...
        <table v-else>
          <thead>
            <tr><th>Item</th><th>Cost</th><th class="text-right">Actions</th></tr>
          </thead>
          <tbody>
      ...
      <div v-if="activeAdjustId" class="modal-overlay" @click.self="closeModal">
        <div class="modal-card">
          ...
          <div class="modal-body">
            <div class="input-field"><label>Amount (€)</label><input type="number" v-model="adjustAmount" step="0.01"></div>
            ...
            <div class="preview-box" v-if="adjustAmount !== 0">
              ...
              <div class="preview-item highlight"><span>If Added:</span><span class="pos">€{{ previewBalance.ifAdded.toFixed(2) }}</span></div>
              <div class="preview-item highlight"><span>If Subtracted:</span><span class="neg">€{{ previewBalance.ifSubtracted.toFixed(2) }}</span></div>
            </div>
          </div>
...
<style scoped>
.admin-view { max-width: 900px; margin: 0 auto; padding-bottom: 50px; }
.tile { background: white; border-radius: 16px; padding: 25px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); margin-bottom: 25px; }
...
.add-wish-form input[type="text"] { flex: 2; }
.cost-input-wrapper { flex: 1; position: relative; display: flex; align-items: center; }
...
.cost-input-wrapper input { padding-left: 30px; width: 100%; }
.currency-prefix { position: absolute; left: 12px; font-weight: bold; color: #747d8c; }

/* Tables */
table { width: 100%; border-collapse: collapse; }
th { text-align: left; font-size: 0.75rem; color: #a0a0a0; text-transform: uppercase; padding: 10px; }
td { padding: 12px 10px; border-bottom: 1px solid #f9f9f9; }
.text-right { text-align: right; }
.actions-cell { display: flex; justify-content: flex-end; gap: 10px; align-items: center; }

/* Buttons */
button { height: 45px; padding: 0 20px; border-radius: 10px; font-weight: bold; cursor: pointer; border: none; transition: transform 0.1s; }
button:active { transform: scale(0.98); }

.btn-success { background: #2ed573; color: white; }
.btn-danger { background: #ff4757; color: white; }
.btn-mini-buy { background: #4a90e2; color: white; height: 32px; padding: 0 12px; font-size: 0.85rem; }
.btn-mini-cancel { background: #ff4757; color: white; height: 32px; padding: 0 12px; font-size: 0.85rem; }
.btn-secondary { background: #f1f2f6; height: 38px; }
.btn-edit-inline { background: none; height: auto; padding: 5px; font-size: 1rem; }
.btn-mini-save { height: 35px; padding: 0 10px; background: #2ed573; color: white; }
.btn-icon-delete { background: none; height: auto; padding: 5px; font-size: 1.2rem; }
...
.modal-card { background: white; width: 400px; border-radius: 20px; padding: 25px; box-shadow: 0 20px 40px rgba(0,0,0,0.2); }
.input-field { margin-bottom: 15px; }
.input-field label { display: block; font-size: 0.8rem; margin-bottom: 5px; color: #747d8c; font-weight: bold; }
